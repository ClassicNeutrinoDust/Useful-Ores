...
public class ArcaniteXpJarRenderer implements net.minecraft.client.renderer.blockentity.BlockEntityRenderer<ArcaniteXpJarBlockEntity> {

    private static final ResourceLocation ORB_TEXTURE =
            ResourceLocation.fromNamespaceAndPath("useful_ores", "textures/entity/arcanite_xp_orb.png");
    ...
    public void render(ArcaniteXpJarBlockEntity be, float partialTick, PoseStack poseStack,
        ...
        for (int i = 0; i < orbCount; i++) {
            ...
            drawOrbBillboard(buffer, poseStack.last(), camRotation, px, py, pz, ORB_HALF_SIZE, rollDeg, rc, bc, FULL_BRIGHT);
        }
    }
    ...
    private static void drawOrbBillboard(VertexConsumer buffer, PoseStack.Pose pose, Quaternionf camRotation,
        ...
        vertex(buffer, pose, SCRATCH_P, cx, cy, cz, 0f, 1f, nx, ny, nz, rc, bc, light);
        SCRATCH_P.set(halfSize, -halfSize, 0f).rotate(rot);
        vertex(buffer, pose, SCRATCH_P, cx, cy, cz, 1f, 1f, nx, ny, nz, rc, bc, light);
        SCRATCH_P.set(halfSize, halfSize, 0f).rotate(rot);
        vertex(buffer, pose, SCRATCH_P, cx, cy, cz, 1f, 0f, nx, ny, nz, rc, bc, light);
        SCRATCH_P.set(-halfSize, halfSize, 0f).rotate(rot);
        vertex(buffer, pose, SCRATCH_P, cx, cy, cz, 0f, 0f, nx, ny, nz, rc, bc, light);
    }

    private static void vertex(VertexConsumer buffer, PoseStack.Pose pose, Vector3f local, float cx, float cy, float cz,
                                float u, float v, float nx, float ny, float nz, int rc, int bc, int light) {
        buffer.addVertex(pose, local.x() + cx, local.y() + cy, local.z() + cz)
