...
final class GlowCubeUtil {
    ...
    static void renderCoreGlow(MultiBufferSource bufferSource, PoseStack.Pose pose, ResourceLocation gemOnTexture,
                                int r, int g, int b, int a) {
        RenderType renderType = RenderType.entityTranslucentEmissive(gemOnTexture);
    ...
    private static void quad(VertexConsumer buffer, PoseStack.Pose pose, int r, int g, int b, int a,
                              float x1, float y1, float z1, float x2, float y2, float z2,
                              float x3, float y3, float z3, float x4, float y4, float z4,
                              float nx, float ny, float nz,
                              float u0, float v0, float u1, float v1) {
        buffer.addVertex(pose, x1, y1, z1).setColor(r, g, b, a).setUv(u0, v0)
                .setOverlay(OverlayTexture.NO_OVERLAY).setLight(FULL_BRIGHT).setNormal(pose, nx, ny, nz);
        buffer.addVertex(pose, x2, y2, z2).setColor(r, g, b, a).setUv(u0, v1)
                .setOverlay(OverlayTexture.NO_OVERLAY).setLight(FULL_BRIGHT).setNormal(pose, nx, ny, nz);
        buffer.addVertex(pose, x3, y3, z3).setColor(r, g, b, a).setUv(u1, v1)
                .setOverlay(OverlayTexture.NO_OVERLAY).setLight(FULL_BRIGHT).setNormal(pose, nx, ny, nz);
        buffer.addVertex(pose, x4, y4, z4).setColor(r, g, b, a).setUv(u1, v0)
                .setOverlay(OverlayTexture.NO_OVERLAY).setLight(FULL_BRIGHT).setNormal(pose, nx, ny, nz);
    }
}
