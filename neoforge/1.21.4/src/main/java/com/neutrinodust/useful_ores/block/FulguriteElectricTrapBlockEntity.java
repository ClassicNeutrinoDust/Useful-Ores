...
public class FulguriteElectricTrapBlockEntity extends BlockEntity implements software.bernie.geckolib.animatable.GeoAnimatable {
   ...
   public FulguriteElectricTrapBlockEntity(BlockPos pos, BlockState state) {
      super(ModBlockEntities.FULGURITE_ELECTRIC_TRAP.get(), pos, state);
   }
   ...
   private static final String TRAP_OWNER_TAG = "useful_ores_trap_owner";
   ...
   private void ensureRippleRingsExist(ServerLevel level, BlockPos pos, double groundY, double centerX, double centerZ) {
      ...
      while (this.rippleRingIds.size() < RIPPLE_COUNT) {
         ...
         display.getPersistentData().putString(TRAP_OWNER_TAG, this.getBlockPos().toString());
   ...
   private void clearRippleRings(ServerLevel level) {
      ...
      this.rippleTick = 0;
      sweepOrphanRings(level, this.worldPosition);
   }
   ...
   private void sweepOrphanRings(ServerLevel level, BlockPos pos) {
      ...
      for (Display.ItemDisplay display : level.getEntitiesOfClass(Display.ItemDisplay.class, area)) {
         ...
         String owner = NbtCompat.getStringOr(display.getPersistentData(), TRAP_OWNER_TAG, "");
         if (owner.isEmpty() || owner.equals(pos.toString())) {
            display.discard();
         }
      }
   }
   ...
   protected void loadAdditional(CompoundTag input, HolderLookup.Provider registries) {
      ...
      this.lightningUnlocked = NbtCompat.getBooleanOr(input, "LightningUnlocked", false);
      this.range = Math.max(1, Math.min(MAX_RANGE, NbtCompat.getIntOr(input, "Range", 1)));
   }
