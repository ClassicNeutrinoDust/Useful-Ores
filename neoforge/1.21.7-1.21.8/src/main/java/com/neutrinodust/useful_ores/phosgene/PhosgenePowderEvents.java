...
public final class PhosgenePowderEvents {
    ...
    public void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        ResourceLocation potionId = contents.potion().flatMap(h -> h.unwrapKey().map(k -> k.location())).orElse(null);
    ...
    private static void tickLevel(ServerLevel level) {
        ...
        for (Map.Entry<Long, PhosgenePowderData.Applied> entry : data.snapshot().entrySet()) {
            ...
            var holder = BuiltInRegistries.POTION.get(ResourceLocation.parse(applied.potionId()));
