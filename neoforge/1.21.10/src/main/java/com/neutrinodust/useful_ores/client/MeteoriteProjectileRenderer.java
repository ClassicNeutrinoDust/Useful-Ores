...
public class MeteoriteProjectileRenderer<T extends MeteoriteProjectile> extends EntityRenderer<T, MeteoriteProjectileRenderer.MeteoriteRenderState> {
    ...
    @Override
    public void submit(MeteoriteRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitBlock(poseStack, ModItems.METEORITE_BLOCK.get().defaultBlockState(), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor);
        poseStack.popPose();
    }
