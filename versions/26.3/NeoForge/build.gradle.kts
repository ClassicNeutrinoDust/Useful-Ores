...
dependencies {
    // GeckoLib for NeoForge 26.3. Loaded from a local jar dropped in mod/libs/ since this
    // build isn't published on a resolvable Maven feed yet. Picked up by wildcard so the
    // exact filename/version doesn't matter - just drop the real geckolib-neoforge-*.jar
    // (5.5.6+ for 26.3) into libs/ and remove any other geckolib jars from that folder.
    //
    ...
    // Sodium's real classes. Update this to the 26.3 build of Sodium's inner
    // jar when it's available - the 26.1.2 jar referenced below will not
    // resolve against 26.3 Sodium's actual class list.
