...
public class SperryliteCatalyticVialItem extends PotionItem {
    ...
    public int getMaxStackSize(ItemStack stack) {
        return stack.has(DataComponents.POTION_CONTENTS) ? 1 : 16;
    }
    ...
    private InteractionResult tryFillFromWater(Level level, Player player, ItemStack stack) {
        ...
        if (!level.isClientSide()) {
            // Fill exactly one vial from the stack instead of converting the
            // whole stack of empty vials into filled ones at once.
            if (stack.getCount() > 1) {
                stack.shrink(1);
