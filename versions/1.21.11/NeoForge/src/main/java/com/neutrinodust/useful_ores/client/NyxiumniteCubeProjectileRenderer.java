...
public class NyxiumniteCubeProjectileRenderer<T extends NyxiumniteCubeProjectile> extends EntityRenderer<T, NyxiumniteCubeProjectileRenderer.CubeRenderState> {
    ...
    @Override
    public void submit(CubeRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitBlock(poseStack, ModItems.NYXIUM_BLOCKS.get(0).get().defaultBlockState(), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor);
        poseStack.popPose();
    }
