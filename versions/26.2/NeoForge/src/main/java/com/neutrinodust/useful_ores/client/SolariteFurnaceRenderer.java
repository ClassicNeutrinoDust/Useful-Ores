...
public class SolariteFurnaceRenderer<T extends SolariteFurnaceBlockEntity> implements BlockEntityRenderer121X<T> {
    ...
    private static final Identifier GLOW_MASK = Identifier.fromNamespaceAndPath(
    ...
    public void render(T furnace, float partialTick, PoseStack poseStack,
        ...
        Identifier texture = lit ? TEXTURE_ON : TEXTURE_OFF;
        ...
        renderCube(buffer, light, packedOverlay, cos, sin);
        ...
        if (lit) {
            ...
            VertexConsumer glow = bufferSource.getBuffer(RenderTypes.entityTranslucentEmissive(GLOW_MASK));
            renderCube(glow, FULL_BRIGHT, packedOverlay, cos, sin, r, g, b, alpha);
        }
    }

    private void renderCube(VertexConsumer buffer, int light, int overlay, float cos, float sin) {
        renderCube(buffer, light, overlay, cos, sin, 255, 255, 255, 255);
    }
    ...
    private void renderCube(VertexConsumer buffer, int light, int overlay, float cos, float sin,
        ...
        quad(buffer, light, overlay, red, green, blue, alpha,
    ...
    private void quad(VertexConsumer buffer, int light, int overlay, int red, int green, int blue, int alpha,
                       float x1, float y1, float z1, float x2, float y2, float z2,
                       float x3, float y3, float z3, float x4, float y4, float z4,
                       float nx, float ny, float nz,
                       float u0, float v0, float u1, float v1) {
        buffer.addVertex(x1, y1, z1).setColor(red, green, blue, alpha).setUv(u0, v0)
                .setOverlay(overlay).setLight(light).setNormal(nx, ny, nz);
        buffer.addVertex(x2, y2, z2).setColor(red, green, blue, alpha).setUv(u1, v0)
                .setOverlay(overlay).setLight(light).setNormal(nx, ny, nz);
        buffer.addVertex(x3, y3, z3).setColor(red, green, blue, alpha).setUv(u1, v1)
                .setOverlay(overlay).setLight(light).setNormal(nx, ny, nz);
        buffer.addVertex(x4, y4, z4).setColor(red, green, blue, alpha).setUv(u0, v1)
                .setOverlay(overlay).setLight(light).setNormal(nx, ny, nz);
    }
