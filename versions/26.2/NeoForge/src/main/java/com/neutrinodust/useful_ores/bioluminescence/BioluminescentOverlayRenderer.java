...
public class BioluminescentOverlayRenderer {
    ...
    private static final double ABSOLUTE_MAX_DIST = 512.0;
    ...
    public static void onSubmitCustomGeometry(SubmitCustomGeometryEvent event) {
        ...
        double rd        = Math.min(ABSOLUTE_MAX_DIST,
                                    Math.max(16.0, mc.options.renderDistance().get() * 16.0 + 16.0));
        double rdSq      = rd * rd;
        double halfRdSq  = (rd * 0.5) * (rd * 0.5);

        PoseStack poseStack = event.getPoseStack();
        SubmitNodeBufferSource buffers = new SubmitNodeBufferSource();
        ...
        buffers.flushInto(event.getSubmitNodeCollector(), poseStack);
    }
