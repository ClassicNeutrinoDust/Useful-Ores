...
public class ModRegisters {
   ...
   public static DeferredItem<Item> registerSpear(String name, ToolMaterial tool, Supplier<Properties> itemProps) {
      ...
      return registerItem(name + "_spear", p -> new com.neutrinodust.useful_ores.item.BackportSpearItem(
         tool, t[0], t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8], p
      ), itemProps.get());
   }
   ...
   public static List<DeferredItem<Item>> registerItems(
      ...
      return List.of(
         registerItem(name + "_sword",      p -> new Item(tool.applySwordProperties(p, swordattr[0], swordattr[1])), itemProp),
         registerItem(name + "_pickaxe",    p -> new Item(tool.applyToolProperties(p, BlockTags.MINEABLE_WITH_PICKAXE, pickaxeattr[0], pickaxeattr[1])), itemProp),
         ...
         registerItem(name + "_helmet",     p -> new Item(armor.humanoidProperties(p, ArmorType.HELMET)), itemProp),
         registerItem(name + "_chestplate", p -> new Item(armor.humanoidProperties(p, ArmorType.CHESTPLATE)), itemProp),
         registerItem(name + "_leggings",   p -> new Item(armor.humanoidProperties(p, ArmorType.LEGGINGS)), itemProp),
         registerItem(name + "_boots",      p -> new Item(armor.humanoidProperties(p, ArmorType.BOOTS)), itemProp)
      );
   }
   ...
   public static List<DeferredItem<Item>> registerAllItems(
      ...
      return List.of(
         ...
         registerItem(name + "_sword",      p -> new Item(tool.applySwordProperties(p, swordattr[0], swordattr[1])), itemProps.get()),
         registerItem(name + "_pickaxe",    p -> new Item(tool.applyToolProperties(p, BlockTags.MINEABLE_WITH_PICKAXE, pickaxeattr[0], pickaxeattr[1])), itemProps.get()),
         ...
         registerItem(name + "_helmet",     p -> new Item(armor.humanoidProperties(p, ArmorType.HELMET)), itemProps.get()),
         registerItem(name + "_chestplate", p -> new Item(armor.humanoidProperties(p, ArmorType.CHESTPLATE)), itemProps.get()),
         registerItem(name + "_leggings",   p -> new Item(armor.humanoidProperties(p, ArmorType.LEGGINGS)), itemProps.get()),
         registerItem(name + "_boots",      p -> new Item(armor.humanoidProperties(p, ArmorType.BOOTS)), itemProps.get()),
   ...
   public static List<DeferredItem<Item>> registerAllItemsWithSwordAbility(
      ...
      return List.of(
         ...
         registerItem(name + "_ingot",      Item::new, itemProps.get()),
         registerItem(name + "_nugget",     Item::new, itemProps.get()),
         registerItem(name + "_sword",      p -> new Item(tool.applySwordProperties(p, swordattr[0], swordattr[1])) {
            @Override
            public boolean hurtEnemy(net.minecraft.world.item.ItemStack stack,
               ...
               // In 1.21.4 Item.hurtEnemy() defaults to false, which prevents
               // ItemStack from dispatching postHurtEnemy(). Return true to
               // preserve the 1.21.5 reference callback architecture.
