...
public class SolarBatteryItem extends BlockItem {
    ...
    public static int getEnergy(ItemStack stack) {
        Integer value = stack.get(ModSolarComponents.BATTERY_ENERGY.get());
        return value == null ? 0 : Math.max(0, Math.min(MAX_ENERGY, value));
    }

    public static void setEnergy(ItemStack stack, int energy) {
        stack.set(ModSolarComponents.BATTERY_ENERGY.get(), Math.max(0, Math.min(MAX_ENERGY, energy)));
    }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, java.util.List<Component> tooltip, TooltipFlag flag) {
        int pct = Math.round(100.0F * getEnergy(stack) / MAX_ENERGY);
        tooltip.add(Component.translatable("item.useful_ores.solar_battery.charge", pct)
