...
/**
 * 1.21.4 renderer for Useful Ores' custom SuperBeaconBlockEntity.
 *
 * The 1.21.4 BeaconRenderer is specifically typed to BeaconBlockEntity, while
 * SuperBeaconBlockEntity intentionally has its own implementation, so it cannot
 * be registered directly with the vanilla renderer. We reuse vanilla's beam
 * drawing routine while sourcing the custom block entity's beam sections.
 */
public final class SuperBeaconRenderer implements BlockEntityRenderer<SuperBeaconBlockEntity> {
    ...
    /**
     * A beacon beam is not confined to the one-block block entity volume.
     * Use the same tall visibility envelope required by vanilla beacon beam
     * rendering so frustum checks cannot discard the renderer while the beam
     * is still visible.
     */
