...
dependencies {
    ...
    // Local GeckoLib Fabric jar (matches the neoforge build's libs/ jar pattern) instead
    // of GeckoLib's own maven. Drop the geckolib-fabric-<mc_version>-<version>.jar into
    // libs/ - every jar in that folder is picked up automatically below, so the exact
    // filename doesn't matter as long as it's the right jar.
