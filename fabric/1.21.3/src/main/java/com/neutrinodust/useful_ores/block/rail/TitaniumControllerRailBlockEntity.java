...
public class TitaniumControllerRailBlockEntity extends BlockEntity {
    ...
    public TitaniumControllerRailBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.TITANIUM_CONTROLLER_RAIL, pos, state);
    }
    ...
    protected void loadAdditional(CompoundTag input, HolderLookup.Provider registries) {
        ...
        this.lastSeenPower = Math.max(0, Math.min(15, NbtCompat.getIntOr(input, "last_seen_power", 0)));
        this.operatingMode = NbtCompat.getBooleanOr(input, "table_mode", false) ? OperatingMode.TABLE : OperatingMode.CYCLE;
    }
