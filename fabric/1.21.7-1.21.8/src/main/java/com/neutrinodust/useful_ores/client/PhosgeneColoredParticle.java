...
public final class PhosgeneColoredParticle extends RisingParticle {
    ...
    private final double centerX, centerZ, baseY;
    ...
    private PhosgeneColoredParticle(ClientLevel level, double x, double y, double z,
                                    ...
                                    int color, float scale, boolean bubble, long seed) {
        super(level, x, y, z, xd, yd, zd);
        this.setSprite(sprite);
        this.bubble = bubble;
        this.centerX = x; this.centerZ = z; this.baseY = y;
    ...
    public void tick() {
        ...
        this.xd = 0.0; this.zd = 0.0; this.yd = 0.0;
    }

    private static long seed(double x, double y, double z, boolean bubble) {
        long h = Double.doubleToLongBits(x) * 31L + Double.doubleToLongBits(y) * 131L + Double.doubleToLongBits(z) * 521L;
        return h ^ (bubble ? 0x9E3779B97F4A7C15L : 0xC2B2AE3D27D4EB4FL);
    }
    ...
    public static final class MistProvider implements ParticleProvider<PhosgeneEffectParticleOptions> {
        ...
        @Override
        public Particle createParticle(PhosgeneEffectParticleOptions o, ClientLevel level, double x, double y, double z, double xd, double yd, double zd) {
            return new PhosgeneColoredParticle(level, x, y, z, xd, yd, zd, sprites.get(0, 1), o.color(), o.scale(), false, seed(x,y,z,false));
        }
    }
    ...
    public static final class BubbleProvider implements ParticleProvider<PhosgeneBubbleParticleOptions> {
        ...
        @Override
        public Particle createParticle(PhosgeneBubbleParticleOptions o, ClientLevel level, double x, double y, double z, double xd, double yd, double zd) {
            return new PhosgeneColoredParticle(level, x, y, z, xd, yd, zd, sprites.get(0, 1), o.color(), o.scale(), true, seed(x,y,z,true));
        }
    }
}
