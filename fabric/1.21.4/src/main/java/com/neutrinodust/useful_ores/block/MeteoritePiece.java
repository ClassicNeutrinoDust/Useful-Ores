...
public class MeteoritePiece extends StructurePiece {
   ...
   public MeteoritePiece(BlockPos origin, int massRadius, double craterRadius, double coreRadius,
      ...
      super(ModStructurePieceTypes.METEORITE, 0, makeBoundingBox(origin, massRadius, craterRadius, burial));
      this.origin = origin;
   ...
   public MeteoritePiece(StructurePieceSerializationContext context, CompoundTag tag) {
      super(ModStructurePieceTypes.METEORITE, tag);
      this.origin = new BlockPos(NbtCompat.getIntOr(tag, "OX", 0), NbtCompat.getIntOr(tag, "OY", 0), NbtCompat.getIntOr(tag, "OZ", 0));
      this.massRadius = NbtCompat.getIntOr(tag, "MassRadius", 0);
      this.craterRadius = NbtCompat.getDoubleOr(tag, "CraterRadius", 0.0);
      this.coreRadius = NbtCompat.getDoubleOr(tag, "CoreRadius", 0.0);
      this.burial = NbtCompat.getIntOr(tag, "Burial", 0);
      this.lobes = readLobes(NbtCompat.getListOrEmpty(tag, "Lobes", 10));
      this.oreBlobs = readOreBlobs(NbtCompat.getListOrEmpty(tag, "OreBlobs", 10));
   }
   ...
   private static double[][] readLobes(ListTag list) {
      ...
      for (int i = 0; i < list.size(); i++) {
         CompoundTag t = NbtCompat.getCompoundOrEmpty(list, i);
         lobes[i] = new double[]{NbtCompat.getDoubleOr(t, "x", 0.0), NbtCompat.getDoubleOr(t, "y", 0.0), NbtCompat.getDoubleOr(t, "z", 0.0), NbtCompat.getDoubleOr(t, "a", 0.0)};
      }
      return lobes;
   }
